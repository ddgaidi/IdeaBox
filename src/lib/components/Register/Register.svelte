<script lang="ts">
    import { User, Mail, Lock, LogIn } from 'lucide-svelte'; // Icônes pour les champs
    ...
    async function handleSubmit() {
        try {
            ...
            if (!response.ok) {
                ...
                return; // Au lieu de throw, on utilise return
            }
...
<div class="bg-white min-h-screen max-md:pt-32 p-4 sm:p-8 font-sans">
    <header class="mb-12 text-center">
        ...
        <p class="text-xl text-gray-600 mt-2">Rejoignez la communauté et commencez à jouer !</p>
    </header>

    <main class="max-w-md mx-auto">
        <h2 class="text-3xl font-semibold text-gray-800 mb-8 text-center">Créer un Compte</h2>
        ...
        {:else}
            <form on:submit|preventDefault={handleSubmit} class="space-y-6 bg-gray-50 p-8 rounded-xl shadow-lg border border-gray-200">
                ...
                <p class="text-center text-sm text-gray-600">
                    Déjà un compte ?
                    <a href="/login" class="font-medium text-[#8128c9] hover:text-violet-700">
                        Se connecter
                    </a>
                </p>
            </form>
